...
def build_speechlet_response(title, output, reprompt_text, should_end_session):
    return {
        ...
        'card': {
            ...
            'image': {
                'smallImageUrl': '<<S3 URL here>>',
                'largeImageUrl': '<<S3 URL Here>>'
            }
        },
        'reprompt': {
            'outputSpeech': {
...
def get_welcome_response():
    ...
    speech_output = "Hi! Welcome to the Alexa Smart Garden. " \
                    "Please tell one of these options: water garden, temperature, soil moisture or timelapse."
...
def on_intent(intent_request, session):
    ...
    if intent_name == 'PlantWatering':
        return water_garden(intent, session)
...
def get_timelapse(intent, session):
    ...
    speech_output = "I\'ll generate your Smart Garden Timelapse and send to your e-mail!"
    reprompt_text = "Could you repeat, please?"
...
def water_garden(intent, session):
    ...
    client.publish(topic='SmartGarden/WaterRelease', qos=0, payload=json.dumps({"water":"plant"}))
...
def read_s3_data(data_type):
    try:
        ...
        bucket = '<<YOUR DATA BUCKET NAME HERE>>'
...
def send_timelapse_email():
    ...
    s3client = boto3.client('s3', region_name='us-east-1')
    bucket = '<<YOUR TIMELAPSE BUCKET NAME HERE'
    ...
    SENDER = "Sender Name <E-MAIL HERE> - YOU SHOULD VALIDATE YOUR E-MAIL FIRST"
    RECIPIENT = "<<E-MAIL HERE?>>"
